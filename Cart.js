...
const Cart = () => {
  ...
  return (
    <div>
        ...
        {cartItems.map(item => (
          <li key={item._id} className="list-group-item d-flex justify-content-between align-items-center fade-in-up">
            ...
            <div>
              ...
              <button className="btn btn-secondary btn-sm mx-1" onClick={() => decreaseQuantity(item._id)}>-</button>
              <button className="btn btn-primary btn-sm mx-1" onClick={() => addToCart(item)}>+</button>
      ...
      <button className="btn btn-success btn-pulse" onClick={placeOrder}>Place Order</button>
    </div>
  );
};
